...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const maxTrail = Math.max(1, Math.min(64, Math.floor(trailLength)));
    ...
    const filmPass = new ShaderPass(grainShader as any);
    ...
    const resize = () => {
      ...
      material.uniforms.iScale.value = calculateScale(host) * scaleMultiplier;
  ...
  return <div ref={containerRef} className={`ghost-cursor ${className}`.trim()} style={mergedStyle} />;
};
